...
- `var/autoload/ezp_extension.php` and `var/autoload/ezp_kernel.php` are generated.
- `bin/php/ezpgenerateautoloads.php` can be extended to scan
  `extension/<ext>/classes/` and `extension/<ext>/autoloads/`.
- Alternatively, add a runtime autoload in `kernel/classes/autoload.php` (or the
  front-controller) that registers a fallback to `extension/<ext>/classes/`.
