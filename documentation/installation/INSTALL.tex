...
\title{Exponential Basic 2.2 Installation Guide}
...
Exponential Basic is a software package released under the GPL lisence (http://www.gnu.org/copyleft/gpl.html),
...
\end{quote}
Exponential Basic is a content management system, among a lot of other things.
This installation manual will try to cover the job of installing eZ
publish on your server.

Since version 2.2 Exponential Basic has a new possible way to be installed:
without virtual hosts and mod\_rewrite. This makes it possible for
people, who don't have a dedicated server or a specialized Exponential Basic
hoster, to install Exponential Basic on their accounts as long as they have
...
Exponential Basic as it's explained in chapter \ref{chpt: dedicated server}.
...
all needed software and Exponential Basic on it, then chapter \ref{chpt: dedicated server}
is for you.
...
install and manage your Exponential Basic site with ease. Read more about
...
\chapter{\label{chpt: dedicated server}Installing Exponential Basic (standard%
...
For the standard installation of Exponential Basic you will need to have
...
needed if you want to run Exponential Basic in nVH mode. More information
...
\begin{itemize}
\item A database. Currently, Exponential Basic suppports MySQL (http://www.mysql.com)
...
recommended to run the latest Apache release, though Exponential Basic shouldn't
be very picky with the Apache versions. We've used Exponential Basic with
...
windows you can just download the binary. (Exponential Basic uses references
for objects and foreach loops. Only version 4.0.4pl1 and later supports
both of these features satisfactorily.)
\item Exponential Basic (http://developer.ez.no/) verision 2.0 or later stable
releases.
\end{itemize}
...
Exponential Basic version 2.2.2 does not require neither QDom or libxml as
previosly releases did. In this release, Exponential Basic uses it own xml
parser : eZ xml. Optioninal support for libxml will probably be reintroduced
in a future version of Exponential Basic
...
You may find precompiled binaries for your system at the Exponential Basic web site,  \url{http://developer.ez.no}.
...
of PHP to use Exponential Basic on your website. We recommend you use PHP
...
\footnote{Exponential Basic versions prior to 2.1 required magic quotes to be enabled
} onwards magic quotes must be turned off for Exponential Basic to work properly.\\
...
The rewrite rules for Exponential Basic do the following:\\
\\
...
This will be fixed in a later version of Exponential Basic.\\
...
This rewrite rule is required if you want to connect to your Exponential Basic
site with Exponential Basic Desktop Edition. Exponential Basic Desktop Edition is
...
\section{Exponential Basic Installation}


\subsection{Program Files}

The next step is to install the Exponential Basic package in your document
...
When all this is done you need to tell Exponential Basic a little about the
...
can be found in the {}``Exponential Basic Customisation Guide''.
...
\footnote{A new feature in Exponential Basic 2.2 is the possibility of clearing the
...
Important note regarding PostgreSQL support in Exponential Basic:

PostgreSQL has one limitation which is not good for Exponential Basic:

Max length of indentifiers used in the database, table names, column
names etc is default set to 32.

Exponential Basic uses names which sometimes are longer, e.g: eZImageCatalogue\_ImageVariationGroup
...
of Exponential Basic. There should be several files ending with {}``.sql''
...
Exponential Basic has a crontab script.
...
After installing Exponential Basic you can test your site through the URL
...
The next manual you should read is the {}``Exponential Basic Customisation
...
for updates, articles about Exponential Basic and how to work with it, as
...
\item Consider this: all Exponential Basic sites has an admin site, perhaps you
...
\subsection{Exponential Basic 2.2.x}
...
(x is replaced with the current Exponential Basic version). Put createdirs.bat
...
\subsection{Exponential Basic and IIS}
...
\footnote{Since Exponential Basic now can be run without mod\_rewrite you may take
...
errors%
\footnote{The Exponential Basic developers want to point out that this does NOT seem
...
Exponential Basic as described.


\chapter{\label{chpt: nVH}Installing Exponential Basic without virtual hosts and
mod\_rewrite}

Installing Exponential Basic this way has several advantages:
...
\subsubsection{Installing the files for Exponential Basic}

\begin{enumerate}
\item Extract the Exponential Basic files (if you want some example data, extract
...
in the top directory of your Exponential Basic installation.\\
...
\item Create the data structures for Exponential Basic with file \texttt{\footnotesize sql/publish\_mysql.sql}
...
it is much safer then. Otherwise you can still use Exponential Basic, but
...
server and two separate directories for Exponential Basic}
...
ask you all relevant questions and install Exponential Basic in a very secure
way.
\end{document}
